        ...
        for (let i = Number(total) - 1; i >= 0 && nfts.length < 3; i--) {
          try {
            ...
            try {
              ...
              console.log('Fetched tx hash for tokenId', tokenId, '=>', transactionHash);
            } catch {}
            nfts.push({
              tokenId,
                ...
                <ul className="w-full space-y-3">
                  {recentNFTs.map((nft) => (
                    <li key={nft.tokenId} className="flex flex-col sm:flex-row sm:items-center justify-between pb-2 last:pb-0">
                      ...
                      <a
                        href={nft.transactionHash ? `https://testnet.bscscan.com/tx/${nft.transactionHash}` : undefined}
                        target="_blank"
                        rel="noopener noreferrer"
                        className="mt-2 sm:mt-0"
                        style={{ pointerEvents: nft.transactionHash ? 'auto' : 'none', opacity: nft.transactionHash ? 1 : 0.5 }}
                        title={nft.transactionHash ? 'View on Explorer' : 'Transaction hash unavailable'}
                      >
                        <RippleButton size="sm" variant="outline" className="px-4 py-1 font-medium" disabled={!nft.transactionHash}>
                          View on Explorer
                        </RippleButton>
                      </a>
                    </li>
