...
export default function ProfileSettings() {
  ...
  useEffect(() => {
    // Generate a mock CARV ID (e.g., random 8-digit hex)
    const mockId = "carv_" + Math.random().toString(16).slice(2, 10);
    setCarvId(mockId);
  ...
  return (
    <>
      ...
      <GlassCard>
        ...
        {/* CARV_ID Connect Section (Mock) */}
        <div className="mb-6 p-4 bg-gray-900 border border-gray-700 rounded-lg">
          <div className="flex items-center gap-2 mb-2">
            <span className="text-mintiq-primary font-semibold">CARV ID</span>
            {carvConnected && <span className="text-xs bg-green-600/20 text-green-400 px-2 py-0.5 rounded-full">Connected</span>}
          </div>
          <p className="text-gray-400 text-sm mb-2">Your CARV ID is connected.</p>
          <div className="flex gap-2 items-center">
            <input
              type="text"
              className="w-64 px-3 py-2 bg-gray-800 border border-gray-700 rounded-lg focus:outline-none focus:ring-2 focus:ring-mintiq-primary"
              value={carvId}
              disabled
            />
            <RippleButton disabled>Connect</RippleButton>
          </div>
        </div>
