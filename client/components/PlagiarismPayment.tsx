...
// Add standard ERC20 ABI for balanceOf
const ERC20_ABI = [
  {
    "constant": true,
...
const NETWORK_CONFIG = {
  chainId: process.env.NEXT_PUBLIC_CHAIN_ID || '0x61', // Default to BNB testnet chainId
  chainName: process.env.NEXT_PUBLIC_CHAIN_NAME || 'BNB Smart Chain Testnet',
  nativeCurrency: {
    name: process.env.NEXT_PUBLIC_CURRENCY_NAME || 'BNB',
    symbol: process.env.NEXT_PUBLIC_CURRENCY_SYMBOL || 'BNB',
    decimals: 18
  },
  rpcUrls: [process.env.NEXT_PUBLIC_RPC_URL!],
  blockExplorerUrls: [process.env.NEXT_PUBLIC_BLOCK_EXPLORER_URL || 'https://testnet.bscscan.com']
};

const BNB_TOKEN_ADDRESS = process.env.NEXT_PUBLIC_PAYWALL_CONTRACT_ADDRESS!;
...
export default function PlagiarismPayment({ onPaymentSuccess, onPaymentError }: PlagiarismPaymentProps) {
  ...
  const initializeContract = async () => {
    ...
    try {
          ...
          if (switchError.code === 4902) {
            ...
            try {
              ...
              if (addError.code === 4001) {
                ...
                throw new Error(`Failed to add BNB Chain network: ${addError.message || 'Unknown error'}`);
              }
            }
          } else {
            throw new Error(`Failed to switch to BNB Chain network: ${switchError.message || 'Unknown error'}`);
          }
        }
      }
      ...
      const tokenContract = new ethers.Contract(BNB_TOKEN_ADDRESS, ERC20_ABI, provider);
      let tokenBalance = '0';
      try {
        ...
        setBalance('0');
        setDebugInfo('Token contract does not support balanceOf or is not ERC20.');
      }
      ...
      try {
        ...
        console.log('Raw balance:', bnbBalance.toString());
        setDebugInfo(`Raw balance: ${bnbBalance.toString()}`);
        
        const formattedBalance = ethers.formatEther(bnbBalance);
  ...
  const handleBuyTokens = async () => {
    ...
    try {
      ...
      // 0.000062 BNB Token for one plagiarism check
      const amountInWei = ethers.parseUnits('0.000062', 18); // 0.000062 BNB token with 18 decimals
      ...
      if (receipt.status === 1) {
        ...
        // Update balance after transaction (use native BNB balance)
  ...
  return (
    <div className="max-w-md mx-auto p-6 bg-white rounded-lg shadow-lg">
      ...
      <div className="mb-4">
        <p className="text-gray-600">Your Token Balance: {balance} BNB</p>
        <p className="text-gray-600">Connected Account: {account.slice(0, 6)}...{account.slice(-4)}</p>
        <p className="text-sm text-gray-500 mt-2">Cost: 0.000062 BNB Token per check</p>
        <p className="text-xs text-gray-400 mt-1">Using BNB Chain Testnet Network</p>
        <p className="text-xs text-gray-400 mt-1">Token Contract: {BNB_TOKEN_ADDRESS.slice(0, 6)}...{BNB_TOKEN_ADDRESS.slice(-4)}</p>
        <p className="text-xs text-gray-400 mt-1">Note: BNB is both the gas token and governance token for the network</p>
      </div>
      ...
      >
        {loading ? 'Processing...' : 'Buy BNB Token for Plagiarism Check'}
      </button>
      ...
      <div className="mt-4 p-3 bg-gray-50 border border-gray-200 rounded-md">
        ...
        <div className="text-xs text-gray-500 mt-1 space-y-1">
          ...
          <p>Token Address: {BNB_TOKEN_ADDRESS}</p>
          <p>Connected Account: {account || 'Not connected'}</p>
          <p>Current Balance: {balance} BNB</p>
